...
To run the server, type `node ./server` inside the project directory. After that, you could access the web-server from your browser by typing `http://localhost:3000/` inside the address bar (feel free to change the port inside the server source code to any other value you like).
